...
name = "twopi_indist_p00"
p1 = o.PseudoScalarField(["p",0,0],modified_momentum_trafo=True)
p2 = o.PseudoScalarField(["-p",0,0],modified_momentum_trafo=True)
...
pip00 = r.rep_from_action(O_h,b1,"twopi_indist_p00")
pip00.check_if_homomorphism()
## tests ##
f = open(filepath + "tests/" + name + "_trafos.txt", "w")
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
assert t.test_matrices_against_LinearCombinations(pip00)
f.write("Test successful.\n")

subspaces = r.study_irreps(pip00,O_h,filepath + "summary_irreps/" + name + ".txt")
subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
trafoss = t.test_trafos_matmul(subspaces_ordered_by_space,["Rot0","Rot1","Rot2","Inv"],pip00)
t.export_vectors(subspaces_ordered_by_space,filepath + name + "_vecdata", real = True)                # for plotting
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,pip00)
subspaces_LC_labelled = t.label_all(subspaces_ordered_by_space,pip00)
...
for i in range(len(pip00.basis)):
    f.write(str(i+1)+":\n")
    f.write(pip00.basis[i].name_gpt)
    f.write("\n")
...
for irrep in subspaces_ordered_by_space:
    for i in range(len(subspaces_ordered_by_space[irrep])):
        ...
        modified_labelled_vecs = t.label(vecs_modified_values,irrep,pip00)
        add_candidates = None
        if "E" in irrep:
