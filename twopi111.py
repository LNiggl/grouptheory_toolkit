...
p1 = o.PseudoScalarField([1,1,1],explicit_momentum_trafo=True)
p2 = o.PseudoScalarField([-1,-1,-1],explicit_momentum_trafo=True)
...
pi111 = r.rep_from_action(O_h,b1,"pi111")

## tests ##
f = open("../tests/twopi111_test_trafos.txt","w")
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
assert t.test_matrices_against_LinearCombinations(pi111)
f.write("Test successful.\n")

subspaces = r.study_irreps(pi111,O_h,"../results/twopi111_irreps.txt")
subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,pi111)
subspaces_LC_labelled = t.label_all(subspaces_ordered_by_space,pi111)
...
for i in range(len(pi111.basis)):
    f.write(str(i+1)+":\n")
    f.write(pi111.basis[i].name_gpt)
    f.write("\n")
f.write("Matrices of rotations and parity:\n")
for A in gen_actions:
    f.write(A + ":\n")
    f.write(str(pi111.hom[A]) + "\n")
