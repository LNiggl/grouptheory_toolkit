...
pippp = r.rep_from_action(O_h,b1,"twopi_indist_ppp")
pippp.check_if_homomorphism()
...
trafoss = t.test_trafos_matmul(subspaces_ordered_by_space,["Rot0","Rot1","Rot2","Inv"],pippp)
...
for irrep in subspaces_ordered_by_space:
    for i in range(len(subspaces_ordered_by_space[irrep])):
        ...
        modified_labelled_vecs = t.label(vecs_modified_values,irrep,pippp)
        add_candidates = None
        if "E" in irrep:
