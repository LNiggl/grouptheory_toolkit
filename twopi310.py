...
p1 = o.PseudoScalarField([3,1,0],explicit_momentum_trafo=True)
p2 = o.PseudoScalarField([-3,-1,0],explicit_momentum_trafo=True)
...
pi310 = r.rep_from_action(O_h,b1,"pi310")

## tests ##
f = open("../tests/twopi310_test_trafos.txt","w")
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
assert t.test_matrices_against_LinearCombinations(pi310)
f.write("Test successful.\n")

subspaces = r.study_irreps(pi310,O_h,"../results/twopi310_irreps.txt")
subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,pi310)
subspaces_LC_labelled = t.label_all(subspaces_ordered_by_space,pi310)
...
for i in range(len(pi310.basis)):
    f.write(str(i+1)+":\n")
    f.write(pi310.basis[i].name_gpt)
    f.write("\n")
f.write("Matrices of rotations and parity:\n")
for A in gen_actions:
    f.write(A + ":\n")
    f.write(str(pi310.hom[A]) + "\n")
