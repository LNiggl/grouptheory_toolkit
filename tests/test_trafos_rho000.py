...
##### rho(0,0,0) particle #####
f = open("../tests/rho000_test_trafos.txt","w")
rho = o.VectorField([0,0,0],[1,0,0])
rho.set_name_gpt(rho.namestring_gpt_rho("minus"))
print(rho.name_gpt)
...
b1 = o.generate_basis(rho,O_h)
rho000 = r.rep_from_action(O_h,b1,"rho000")

weights_rho000 = []
for i in range(len(rho000.basis)):
    basisv = [0 for j in range(len(rho000.basis))]
    basisv[i] = 1
    weights_rho000.append(basisv)
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
for i in range(len(weights_rho000)):
    assert t.matrices_agree_with_LinComb_objects(rho000,weights_rho000[i])
f.write("Test successful.\n")

subspaces = r.study_irreps(rho000,O_h,"../results/rho000_irreps.txt")
print(subspaces)
### check if invariant irred. subspaces are independent of each other

subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,rho000)
...
for i in range(len(rho000.basis)):
    f.write(str(i+1)+":\n")
    f.write(rho000.basis[i].name_gpt)
    f.write("\n")
f.write("Matrices of rotations and parity:\n")
for A in gen_actions:
    f.write(A + ":\n")
    f.write(str(rho000.hom[A]) + "\n")
#######################################
f.write("\nInvariant irreducible subspaces:\n\n")
f.write("T1m\n")

LC_T1m_space_1 = t.T1_labelling(subspaces_ordered_by_space["T1m"][0],rho000)
