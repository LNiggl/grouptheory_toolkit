...
p1 = o.PseudoScalarField([3,2,2],explicit_momentum_trafo=True)
p2 = o.PseudoScalarField([-3,-2,-2],explicit_momentum_trafo=True)
...
pi322 = r.rep_from_action(O_h,b1,"pi322")

## tests ##
f = open("../tests/twopi322_test_trafos.txt","w")
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
assert t.test_matrices_against_LinearCombinations(pi322)
f.write("Test successful.\n")

subspaces = r.study_irreps(pi322,O_h,"../results/twopi322_irreps.txt")
subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,pi322)
subspaces_LC_labelled = t.label_all(subspaces_ordered_by_space,pi322)
...
for i in range(len(pi322.basis)):
    f.write(str(i+1)+":\n")
    f.write(pi322.basis[i].name_gpt)
    f.write("\n")
f.write("Matrices of rotations and parity:\n")
for A in gen_actions:
    f.write(A + ":\n")
    f.write(str(pi322.hom[A]) + "\n")
