...
pipp0 = r.rep_from_action(O_h,b1,"twopipp0")

## tests ##
f = open(filepath + "tests/" + name + "_trafos.txt", "w")
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
assert t.test_matrices_against_LinearCombinations(pipp0)
f.write("Test successful.\n")

subspaces = r.study_irreps(pipp0,O_h,filepath + "summary_irreps/" + name + ".txt")
subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
t.export_vectors(subspaces_ordered_by_space,filepath + name + "_vecdata", real = True) # D:/Master/Masterarbeit/results/test_01/twopipp0_vecdata", real = True)
# import sys
# sys.exit()
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,pipp0)
subspaces_LC_labelled = t.label_all(subspaces_ordered_by_space,pipp0)
...
for i in range(len(pipp0.basis)):
    f.write(str(i+1)+":\n")
    f.write(pipp0.basis[i].name_gpt)
    f.write("\n")
# f.write("Matrices of rotations and parity:\n")
# for A in gen_actions_O_h:
#     f.write(A + ":\n")
#     f.write(str(pipp0.hom[A]) + "\n")
