...
name = "twopi_indist_pq0"
p1 = o.PseudoScalarField(["p","q",0],modified_momentum_trafo=True)
p2 = o.PseudoScalarField(["-p","-q",0],modified_momentum_trafo=True)
...
pipq0 = r.rep_from_action(O_h,b1,"twopi_indist_p00")
pipq0.check_if_homomorphism()

## tests ##
f = open(filepath + "tests/" + name + "_trafos.txt", "w")
f.write("Test: LinearCombination action agrees with matrix multiplication:   ")
assert t.test_matrices_against_LinearCombinations(pipq0)
f.write("Test successful.\n")

subspaces = r.study_irreps(pipq0,O_h,filepath + "summary_irreps/" + name + ".txt")
subspaces_ordered_by_space = t.reorder_subspaces(subspaces)
trafoss = t.test_trafos_matmul(subspaces_ordered_by_space,["Rot0","Rot1","Rot2","Inv"],pipq0)
t.export_vectors(subspaces_ordered_by_space,filepath + name + "_vecdata", real = True)                # for plotting
all_disjoint = t.subspaces_disjoint(subspaces_ordered_by_space,pipq0)
subspaces_LC_labelled = t.label_all(subspaces_ordered_by_space,pipq0)
...
for i in range(len(pipq0.basis)):
    f.write(str(i+1)+":\n")
    f.write(pipq0.basis[i].name_gpt)
    f.write("\n")
...
for irrep in subspaces_ordered_by_space:
    for i in range(len(subspaces_ordered_by_space[irrep])):
        ...
        modified_labelled_vecs = t.label(vecs_modified_values,irrep,pipq0)
        add_candidates = None
        if "E" in irrep:
